...
- **Check whether a study has been superseded.** METR's early-2025 slowdown study now carries an "out of date" banner pointing to a 2026 update; the Developer page cited the old result for months. Before reusing a source, check the page for update notices, and cite the newer statement with its own caveats.
- **Paywalled or bot-blocked papers (ACM DL) can be read with Playwright's Chromium** (`--headless=new`, normal user agent) when curl and WebFetch get 403. Don't accept "unverifiable" for claims in a paper you can open in a browser.
